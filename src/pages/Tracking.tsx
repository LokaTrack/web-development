...
const TrackerItem = styled(ListItem)<{ selected: boolean }>(({ theme, selected }) => ({
  ...
  '& .MuiListItemText-secondary': {
    color: selected ? 'rgba(255, 255, 255, 0.85)' : theme.palette.text.secondary,
  },
...
const Tracking = () => {
  ...
  const apiUrl = "https://lokatrack.me/api/v1/trackers/";
  const accessToken = "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiJiYWEzZGZhNi0yMDJhLTRlY2UtYWRmMi0yYTc4MjRiMDc2YjkiLCJlbWFpbCI6ImJ1ZGkuZHJpdmVyQGxva2F0cmFjay5jb20iLCJyb2xlIjoiZHJpdmVyIiwidXNlcm5hbWUiOiJCdWRpIExva2F0cmFjayIsImV4cCI6MTc0NjI5NjUyMCwiaXNzIjoiaHR0cHM6Ly9sb2thdHJhY2subWUifQ.kwtXzFOhmgzUvHisRQAdF0rsRRok-OhF4MFCT-aZCTg";
  ...
  const fetchTrackerLocation = async (trackerId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}${trackerId}`, {
        method: 'GET',
        headers: {
      ...
      if (data.status === 'success' && data.data) {
        ...
        setMapCenter([data.data.location.latitude, data.data.location.longitude]);
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch tracker location');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim() !== '') {
      ...
      const filtered = mockTrackers.filter(tracker => 
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" sx={{ mb: 3 }}>Live Tracking</Typography>
      
      <Grid container spacing={3}>
        {/* Tracker list sidebar */}
        <Grid item xs={12} md={4} lg={3}>
          <Paper sx={{ p: 2, borderRadius: 3, height: '78vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <Typography variant="h6" sx={{ mb: 2 }}>Trackers</Typography>
                ...
                <Box>
                  ...
                  >
                    <ListItemAvatar>
                      <Avatar 
                        sx={{ 
                          bgcolor: selectedTrackerId === 'lokatrack-gps-1' ? 'white' : 'primary.main',
                          color: selectedTrackerId === 'lokatrack-gps-1' ? 'success.main' : 'white',
                        }}
                      >
                        <MyLocation />
                      </Avatar>
                    ...
                    <ListItemText 
                      primary={trackerLocation?.trackerName || "GPS Tracker lokatrack-gps-1"}
                      secondary={
                        <Box sx={{ mt: 0.5, display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                          <Typography 
                            ...
                            sx={{ 
                              color: 'inherit',
                              fontWeight: selectedTrackerId === 'lokatrack-gps-1' ? 'medium' : 'normal' 
                            }}
                          >
                            ID: lokatrack-gps-1
                          </Typography>
                          ...
                          >
                            Driver: {trackerLocation?.username || "Budi Lokatrack"}
                          </Typography>
                          <Box 
                            sx={{ 
                              mt: 0.5, 
                              backgroundColor: selectedTrackerId === 'lokatrack-gps-1' ? 'rgba(255,255,255,0.2)' : 'transparent',
                              borderRadius: 1,
                  ...
                  <Box key={tracker.id}>
                    ...
                    >
                      <ListItemAvatar>
                        <Avatar 
                          sx={{ 
                            bgcolor: selectedTrackerId === tracker.id ? 'white' : 'primary.main',
                            color: selectedTrackerId === tracker.id ? 'success.main' : 'white',
                          }}
                        >
                          <MyLocation />
                        </Avatar>
                        ...
                        secondary={
                          <Box sx={{ mt: 0.5, display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                            <Typography 
                              ...
                              sx={{ 
                                color: 'inherit',
                                fontWeight: selectedTrackerId === tracker.id ? 'medium' : 'normal' 
                              }}
                            >
                              ID: {tracker.deviceId}
                            </Typography>
                            {tracker.assignedTo ? (
                              ...
                              <Typography 
                                variant="body2" 
                                sx={{ 
                                  color: selectedTrackerId === tracker.id ? 'rgba(255, 255, 255, 0.8)' : 'error.main',
                                  fontWeight: selectedTrackerId === tracker.id ? 'medium' : 'normal' 
                                }}
                              >
                                Not assigned
                            ...
                            <Box 
                              sx={{ 
                                mt: 0.5, 
                                backgroundColor: selectedTrackerId === tracker.id ? 'rgba(255,255,255,0.2)' : 'transparent',
                                borderRadius: 1,
                                display: 'inline-block',
                                px: 0.5
                              }}
        ...
        <Grid item xs={12} md={8} lg={9}>
          <Paper sx={{ p: 0, borderRadius: 3, height: '78vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            {/* Loading indicator */}
            {isLoading && (
              <LinearProgress color="primary" sx={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 1001 }} />
            )}
            ...
            <Box sx={{ 
              ...
              <Typography variant="caption" color="text.secondary">
                {trackerLocation ? `Last update: ${formatTimestamp(trackerLocation.lastUpdate)}` : 'No data'}
              </Typography>
            ...
            <Box sx={{ flexGrow: 1, position: 'relative', zIndex: 0 }}>
              ...
              >
                ...
                {trackerLocation && (
                  ...
                  >
                    <Popup>
                      <div style={{ fontFamily: 'Arial, sans-serif', padding: '8px 0' }}>
                        <h3 style={{ margin: '0 0 8px 0', color: '#1976d2' }}>{trackerLocation.trackerName}</h3>
                        <div style={{ padding: '8px', backgroundColor: '#f5f5f5', borderRadius: '4px', marginBottom: '8px' }}>
                          <p style={{ margin: '4px 0', fontSize: '14px' }}><strong>Driver:</strong> {trackerLocation.username}</p>
                          <p style={{ margin: '4px 0', fontSize: '14px' }}><strong>Phone:</strong> {trackerLocation.phoneNumber}</p>
                        </div>
                        <p style={{ margin: '4px 0', fontSize: '14px' }}><strong>Last Update:</strong><br/>{formatTimestamp(trackerLocation.lastUpdate)}</p>
                        <p style={{ margin: '4px 0', fontSize: '14px' }}>
                          <strong>Coordinates:</strong><br/>
                          {Math.abs(trackerLocation.latitude).toFixed(6)}° {trackerLocation.latitude < 0 ? 'S' : 'N'}, 
                          {Math.abs(trackerLocation.longitude).toFixed(6)}° {trackerLocation.longitude < 0 ? 'W' : 'E'}
                        </p>
                      </div>
                    </Popup>
                  </Marker>
              ...
              <Box sx={{ p: 2, backgroundColor: 'background.paper', borderTop: 1, borderColor: 'divider' }}>
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={6}>
                    ...
                    <Box sx={{ mt: 1, p: 1.5, bgcolor: 'background.default', borderRadius: 2, border: '1px solid rgba(0,0,0,0.1)' }}>
                      <Typography variant="body2" sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                        <span>Driver:</span> <strong style={{ color: '#2e7d32' }}>{trackerLocation.username || 'Not assigned'}</strong>
                      </Typography>
                      <Typography variant="body2" sx={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>Phone:</span> <strong style={{ color: '#2e7d32' }}>{trackerLocation.phoneNumber || 'No phone number'}</strong>
                      </Typography>
                    </Box>
                    <Typography variant="body2" sx={{ mt: 1 }}>
                      Registration Date: <strong>{formatTimestamp(trackerLocation.lastUpdate)}</strong>
                    </Typography>
                  </Grid>
                      ...
                      <Grid item xs={12}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Box>
                            ...
                            <Typography variant="caption" color="text.secondary">
                              Last updated {formatTimestamp(trackerLocation.lastUpdate)}
                            </Typography>
                          </Box>
                        </Box>
                      </Grid>
